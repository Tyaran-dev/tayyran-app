...
//         case "AUTHORIZED":
//           debugPrint(
//             "🟦 Payment is AUTHORIZED - Will check again in 8 seconds",
//           );
//           emit(
//             PaymentStatusAuthorized(
//               invoiceId: invoiceId,
//               message: "Payment_authorized".tr(),
//               timerCount: _timerCount,
//               checkAttempts: _checkAttempts,
//               startTime: _startTime,
//             ),
...
//             ),
//           );
//         } else if (currentState is PaymentStatusAuthorized) {
//           debugPrint(
//             "🔵 Authorized state update - Timer: $_timerCount, Next check in: ${8 - (_timerCount % 8)} seconds",
//           );
//           emit(
//             PaymentStatusAuthorized(
//               invoiceId: invoiceId,
...
class PaymentStatusCubit extends Cubit<PaymentStatusState> {
  ...
  static const int _maxRetryAttempts = 30; // ~4 minutes (30 * 8 seconds)
  static const int _maxDurationSeconds = 300; // 5 minutes maximum
  ...
  Future<void> _checkPaymentStatus() async {
    ...
    if (_shouldStopMonitoring()) {
      debugPrint(
        "🟦 Maximum retry attempts or time reached - stopping monitoring",
      );
      _statusCheckTimer?.cancel();
      emit(
        PaymentStatusError(
          invoiceId: invoiceId,
          errorMessage: "paymentStatus.timeout".tr(),
          timerCount: _timerCount,
          checkAttempts: _checkAttempts,
          startTime: _startTime,
        ),
    ...
    try {
      ...
      if (!_isValidStatus(status)) {
        debugPrint("🟧 Invalid status received: $status");
        emit(
          PaymentStatusError(
            invoiceId: invoiceId,
            errorMessage: "paymentStatus.invalidStatus".tr(),
            timerCount: _timerCount,
            checkAttempts: _checkAttempts,
            startTime: _startTime,
          ),
      ...
      switch (status.toUpperCase()) {
        case "PENDING":
          ...
          emit(
            PaymentStatusPending(
              invoiceId: invoiceId,
              message: "paymentStatus.payment_pending".tr(),
              timerCount: _timerCount,
              checkAttempts: _checkAttempts,
              startTime: _startTime,
            ),
        ...
        case "AUTHORIZED":
          ...
          emit(
            PaymentStatusAuthorized(
              invoiceId: invoiceId,
              message: "paymentStatus.payment_authorized".tr(),
              timerCount: _timerCount,
              checkAttempts: _checkAttempts,
              startTime: _startTime,
            ),
        ...
        case "CONFIRMED":
          ...
          _sendConfirmationEmail(paymentStatusResponse);
        ...
        case "FAILED":
          ...
          emit(
            PaymentStatusFailed(
              invoiceId: invoiceId,
              reason: "paymentStatus.payment_failed".tr(),
              timerCount: _timerCount,
              checkAttempts: _checkAttempts,
              startTime: _startTime,
              totalDuration: DateTime.now().difference(_startTime!).inSeconds,
      ...
      emit(
        PaymentStatusError(
          invoiceId: invoiceId,
          errorMessage: "paymentStatus.payment_failed_status".tr(),
          timerCount: _timerCount,
          checkAttempts: _checkAttempts,
          startTime: _startTime,
        ),
  ...
  void _scheduleNextCheck() {
    ...
    _statusCheckTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      // Only emit state updates when NOT about to call API
      if (_timerCount % 8 != 0) {
        final currentState = state;
        final secondsUntilNextCheck = 8 - (_timerCount % 8);

        debugPrint(
          "⏳ State update - Timer: $_timerCount, Next check in: $secondsUntilNextCheck seconds",
        );
  ...
  Future<void> _sendConfirmationEmail(PaymentStatusResponse response) async {
    try {
      ...
      debugPrint("❌ Failed to send confirmation email: $error");
      // Don't rethrow - email failure shouldn't affect payment status
    }
  }
  ...
  bool _shouldStopMonitoring() {
    ...
    return _checkAttempts >= _maxRetryAttempts ||
        duration.inSeconds >= _maxDurationSeconds;
  }

  bool _isValidStatus(String status) {
    final validStatuses = ["PENDING", "AUTHORIZED", "CONFIRMED", "FAILED"];
