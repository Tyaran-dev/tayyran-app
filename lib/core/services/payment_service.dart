...
//         MFEnvironment.LIVE, // Change to LIVE for production
//       );
//       initializeAppBar();
//     } catch (error) {
...
enum PaymentEnvironment { test, live }
...
class PaymentService {
  ...
  static Future<void> init({
    PaymentEnvironment env = PaymentEnvironment.live,
  }) async {
    if (isInitialized) return;

    try {
      final sdkEnv = env == PaymentEnvironment.live
          ? MFEnvironment.LIVE
          : MFEnvironment.TEST;
      final apiKey = env == PaymentEnvironment.live ? _liveKey : _testKey;

      MFSDK.init(apiKey, MFCountry.SAUDIARABIA, sdkEnv);
      ...
      debugPrint('Payment SDK initialization failed: $error');
      throw Exception('Payment SDK initialization failed');
    }
  }
}
